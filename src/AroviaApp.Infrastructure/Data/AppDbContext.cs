...
namespace AroviaApp.Infrastructure.Data;
...
public class AppDbContext : DbContext, IAppDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // Configure Patient - CreatedByUser (One-to-Many)
        modelBuilder.Entity<Patient>()
        ...
        // Global Query Filter for Patients
        // Doctors see only their patients. Admins/Nurses see all patients in clinic. SuperAdmin sees all.
        modelBuilder.Entity<Patient>().HasQueryFilter(p =>
            _currentUserService.Role == "SuperAdmin" ||
            (_currentUserService.ClinicId == p.ClinicId && (_currentUserService.Role == "Admin" || _currentUserService.Role == "Nurse")) ||
            (_currentUserService.UserId == p.CreatedByUserId));

        // Global Query Filter for Appointments
        modelBuilder.Entity<Appointment>().HasQueryFilter(a =>
            _currentUserService.Role == "SuperAdmin" ||
            (_currentUserService.ClinicId == a.ClinicId && (_currentUserService.Role == "Admin" || _currentUserService.Role == "Nurse")) ||
            (_currentUserService.UserId == a.DoctorId));

        // Global Query Filter for MedicalRecords
        modelBuilder.Entity<MedicalRecord>().HasQueryFilter(m =>
            _currentUserService.Role == "SuperAdmin" ||
            (_currentUserService.ClinicId == m.ClinicId && (_currentUserService.Role == "Admin" || _currentUserService.Role == "Nurse")) ||
            (_currentUserService.UserId == m.DoctorId));
    ...
    private void ApplyAuditInfo()
    {
        foreach (var entry in ChangeTracker.Entries<Patient>())
        {
            ...
            {
                if (entry.Entity.CreatedByUserId == Guid.Empty && _currentUserService.UserId.HasValue)
                {
                    entry.Entity.CreatedByUserId = _currentUserService.UserId.Value;
                }
                
                if (entry.Entity.ClinicId == Guid.Empty && _currentUserService.ClinicId.HasValue)
                {
                    entry.Entity.ClinicId = _currentUserService.ClinicId.Value;
                }
            }
        }
    }
